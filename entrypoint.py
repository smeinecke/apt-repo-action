...
if __name__ == '__main__':
    ...
    github_repo = os.environ.get('INPUT_GITHUB_REPOSITORY', os.environ.get('GITHUB_REPOSITORY'))
    ...
    os.system(
        'reprepro -b {} --keepunusednewfiles --export=silent-never includedeb {} {}'.format(
            apt_dir,
            deb_file_version,
            deb_file_path,
        )
    ...
    git_repo.index.commit(
        '[apt-action] Update apt repo\n\n\napt-action-metadata{}\n\ndeplying: {}'.format(current_metadata_str, deploy_commit_sha)
    )
